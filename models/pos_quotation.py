...
class POSQuotation(models.Model):
    ...
    analytic_account_tags = fields.Many2many('account.analytic.tag')
    ...
    def create_quotation(self, vals):
        sequence = self.env.ref('pos_quotation.sequence_quote_sequence')
        if vals.get('analytic_account_tags'):
            vals.update({'analytic_account_tags': [(6, 0, vals['analytic_account_tags'])]})
    ...
    def get_quotation_details(self):
        return {
            ...
            'id': self.id,
            'ref': self.ref,
            'analytic_account_tags': self.analytic_account_tags.ids,
